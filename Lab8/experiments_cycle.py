...
filename = 'эксперименты итератор/'
...
def train_and_evaluate_model(iter, X_train, y_train, X_valid, y_valid, X_test, y_test, df, filename):
    ...
    model = Sequential()
    model.add(Embedding(input_dim=vocab_size, output_dim=16 * maxSequenceLength, input_length=maxSequenceLength))
    ...
    # Обучение модели
    history = model.fit(X_train, y_train, batch_size=batch_size, epochs=500, validation_data=(X_valid, y_valid))

    # Оценка модели на тестовых данных и отчет о качестве
    predictions = model.predict(X_test).argmax(axis=1)
    y_true = np.array(y_test_1)
    report = classification_report(y_true, predictions)
    print(f"Результаты для {iter} :")
    print(report)
    macro_f1 = f1_score(y_true, predictions, average='macro')
    print(f'Macro F1 Score: {macro_f1}')

    global cycle
    cycle.append([float(iter), float(macro_f1)])

    # Сохранение в log.txt
    with open(filename + f'{iter}_log.txt', 'a', encoding='utf-8') as f:
        f.write(f"Model Parameters for {iter} epochs:\n")
